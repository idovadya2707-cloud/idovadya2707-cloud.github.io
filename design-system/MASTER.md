...
|------|-----------------|------|------|--------|
| **User** | inline‑start (right in RTL) | `--surface-selected` `#eef4fe` (or `--color-secondary` fill with white text for high emphasis) | `--text-body` / white | `--radius-lg`, tail corner `--radius-sm` on the start side |
